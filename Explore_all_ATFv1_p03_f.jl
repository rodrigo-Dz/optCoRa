...
function bifurcation_analysis(n_points, a, b, n_params, u0, system_FB, system_nFB, tspan, pOp, p, mY_range)
    ...
    for i in 1:n_points
        ...
        push!(results,  vcat(collect(values(p)), r_01, r_02, r_03, os, raro, ss))
    end

    # Convertir results a una matriz
    results_matrix = reduce(vcat, transpose.(results))
    # Guardar los resultados en un archivo CSV
    writedlm("bifurcation_results_ATFv1_p03_f_pos.csv", results_matrix, ',')
...
mY_range = 10 .^ range(-3, 3, length=10)  # Rango logarítmico para mY: 0.001 a 1000
...
n_points = 8 # cantidad de puntos (recomendable usar una potencia de 2)
n_params = 3
