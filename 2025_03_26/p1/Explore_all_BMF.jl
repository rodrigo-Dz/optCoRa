...
function bifurcation_analysis(n_points, a, b, n_params, u0, system_FB, system_nFB, tspan, pOp, p, mY_range)
    ...
    for i in 1:n_points
        ...
        #sol = solve_to_steady_state(system_FB, next_ci, p, tspan)
            ...
            p = copy(copy_p)
            p[:mBs] = p[:mB] * FB
            sol = find_equilibrium(p, SS, system_nFB)
            ss_= sol[1]
            ...
            if abs(FB - nFB) < 0.0001
                ...
                p = copy(copy_p)
                p[:mY] = p[:mY] * 1.05
                sol = find_equilibrium(p, SS, system_FB)
                ss_= sol[1]
    ...
    writedlm("bifurcation_results_BMF_nlsolver_osc.csv", results_matrix, ',')
...
pOp  = [:mU,:mA,:mB,:eP]	# Parameters to optimize
