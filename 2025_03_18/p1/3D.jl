...
# Bifurcation analysis: Variar mY, gY y mW en escala logarítmica
mY_range = 10 .^ range(-3, 3, length=50)  # Rango logarítmico para mY: 0.001 a 1000
gY_range = 10 .^ range(-3, 3, length=50)  # Rango logarítmico para gY: 0.001 a 1000
mW_range = 10 .^ range(-3, 3, length=50)  # Rango logarítmico para mW: 0.001 a 1000

# Función para realizar el análisis de bifurcación
function bifurcation_analysis(mY_range, gY_range, mW_range, params, u0, system!, system_name, grid, pert)
    bifurcation_data = []  # Almacenar datos de oscilaciones
    for i in 1:length(mY_range), j in 1:length(gY_range), k in 1:length(mW_range)
        params[2] = mY_range[i]  # Actualizar mY
        params[3] = gY_range[j]  # Actualizar gY
        params[6] = mW_range[k]  # Actualizar mW

        if grid != 0
            params[11] = params[4] * grid[k, j, i]
        end
        ...
        push!(bifurcation_data, (mY_range[i], gY_range[j], mW_range[k], steady_state_Y, color))
    end

    # Preparar datos para el heatmap
    mY_values = [data[1] for data in bifurcation_data]
    gY_values = [data[2] for data in bifurcation_data]
    mW_values = [data[3] for data in bifurcation_data]
    steady_state_Y = [data[4] for data in bifurcation_data]
    colors = [data[5] for data in bifurcation_data]

    # Reshape data for heatmap
    mY_grid = reshape(mY_values, (length(mY_range), length(gY_range), length(mW_range)))
    gY_grid = reshape(gY_values, (length(mY_range), length(gY_range), length(mW_range)))
    mW_grid = reshape(mW_values, (length(mY_range), length(gY_range), length(mW_range)))
    steady_state_Y_grid = reshape(steady_state_Y, (length(mY_range), length(gY_range), length(mW_range)))
    color_grid = reshape(colors, (length(mY_range), length(gY_range), length(mW_range)))

    # Gráfica de regiones con oscilaciones
    surface(mY_range, gY_range, mW_range, log10.(steady_state_Y_grid), xlabel="mY", ylabel="gY", zlabel="mW",
            title="Estado estacionario de Y ($system_name)", xscale=:log10, yscale=:log10, zscale=:log10,
...
ss_fb, oscilations = bifurcation_analysis(mY_range, gY_range, mW_range, params, u0, ode_system!, "Sistema_1", 0, 0)

params = p_copy
# Realizar el análisis de bifurcación para el segundo sistema
ss_nf, x = bifurcation_analysis(mY_range, gY_range, mW_range, params, u0, ode_systemNF!, "Sistema_2", ss_fb, 0)

params = p_copy
ss_fb_p, x = bifurcation_analysis(mY_range, gY_range, mW_range, params, u0, ode_system!, "Sistema_1_p", 0, 1)
...
params = p_copy
# Realizar el análisis de bifurcación para el segundo sistema
ss_nf_p, x = bifurcation_analysis(mY_range, gY_range, mW_range, params, u0, ode_systemNF!, "Sistema_2_p", ss_fb, 1)

CoRa = log10.(ss_fb_p./ss_fb) ./ log10.(ss_nf_p./ss_nf)
using Plots

# Graficar con la escala de colores fija en el rango [0,1]
surface(mY_range, gY_range, mW_range, CoRa, xlabel="mY", ylabel="gY", zlabel="mW",
        title="CoRas", xscale=:log10, yscale=:log10, zscale=:log10,
