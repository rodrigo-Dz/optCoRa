...
Threads.@threads for ruN in 1:mrw.runs
    ...
    open(string("OUT_OptCoRa_",iARG.mm,"_",iARG.ex,"_",iARG.pp,"_",iARG.ax,"_",ruN,".txt"), "w") do io
        ...
        # Initialize system:
        CoRa0,ss0 = fn.CoRaCurve(p, pert, mm, n, gap_size, gap_tol, x0, rtol, strict)
        op0, mi0, cost0, w0 = fn.CoRam(CoRa0,eps)
        
        ss_desired = 0.18
        ss_top = ss0*1.5
        ss_bottom = ss_desired*0.5
                # Print initial state:
        ...
        while (i != mrw.iter)  #size(pars, 1)   # Número de filas     
            ...
            # curve
            CoRa1,ss1 =  fn.CoRaCurve(p, pert, mm, n, gap_size, gap_tol, x0, rtol, strict)
